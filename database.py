...
class User(UserMixin, db.Model):
    ...
    def is_license_valid(self):
        ...
        if datetime.utcnow() > self.license_expiry_date: self.license_status = 'expired'; return False
    ...
    def deduct_credits(self, amount, transaction_type='usage', description=''):
        if (self.credits or 0) < amount: return False
        self.credits = (self.credits or 0) - amount
    ...
    def clear_reset_token(self):
        self.reset_token = None; self.reset_token_expiry = None
...
class Device(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('user_id', 'hwid_hash', name='unique_user_device'), db.Index('idx_devices_hwid_hash', 'hwid_hash'), db.Index('idx_devices_user_active', 'user_id', 'is_active'), db.Index('idx_devices_last_seen', 'last_seen'))
    ...
    def create_session(self, expires_in_hours=12, ip_address=None, user_agent=None):
        ...
        session = UserSession(user_id=self.user_id, device_id=self.id, session_token=self.generate_session_token(), expires_at=datetime.utcnow() + timedelta(hours=expires_in_hours), ip_address=ip_address or self.ip_address, user_agent=user_agent)
    ...
    def deactivate(self):
        self.is_active = False; self.is_bound = False
        UserSession.query.filter_by(device_id=self.id, is_active=True).update({'is_active': False})
        db.session.commit()
...
class UserSession(db.Model):
    ...
    is_active = db.Column(db.Boolean, default=True)
    __table_args__ = (db.Index('idx_sessions_user_active', 'user_id', 'is_active', 'expires_at'), db.Index('idx_sessions_token_expiry', 'session_token', 'expires_at'))

    def is_valid(self): return self.is_active and self.expires_at > datetime.utcnow()
    def refresh(self, hours=12): self.expires_at = datetime.utcnow() + timedelta(hours=hours); self.last_activity = datetime.utcnow(); db.session.commit()
    def invalidate(self): self.is_active = False; db.session.commit()
    ...
    def __repr__(self): return f'<UserSession {self.id} for User {self.user_id}>'
...
class DeviceHistory(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)
    __table_args__ = (db.CheckConstraint("action IN ('register','unregister','reset','change_pc','login','logout','admin_deleted','reactivate','session_created','session_expired','admin_reset_all','admin_reset_single','admin_toggle_trust','reset_all','reset_single')", name='check_device_action'), db.Index('idx_device_history_user_date', 'user_id', 'created_at'), db.Index('idx_device_history_action', 'action'))
    ...
    def log_action(cls, user_id, action, device=None, hwid_hash=None, device_name=None, ip_address=None, user_agent=None, reason=None, extra_data=None):
        history = cls(user_id=user_id, device_id=device.id if device else None, hardware_id=device.hardware_id if device else None, hwid_hash=hwid_hash or (device.hwid_hash if device else None), device_name=device_name or (device.device_name if device else None), action=action, reason=reason, ip_address=ip_address, user_agent=user_agent, extra_data=extra_data)
        db.session.add(history); db.session.commit()
        return history

    def __repr__(self): return f'<DeviceHistory {self.action} for User {self.user_id}>'
...
class SystemLog(db.Model):
    ...
    __table_args__ = (db.Index('idx_system_logs_user_created', 'user_id', 'created_at'),)
    def __repr__(self): return f'<SystemLog {self.log_type}>'
...
class CreditTransaction(db.Model):
    ...
    def __repr__(self): return f'<CreditTransaction {self.id}: {self.amount}>'
...
class ResellerCommission(db.Model):
    ...
    __table_args__ = (db.CheckConstraint("license_type IN ('Fair','Good','Excellent')", name='check_commission_license_type'), db.Index('idx_commissions_reseller_date', 'reseller_id', 'created_at'))
    def __repr__(self): return f'<ResellerCommission {self.id}: {self.commission}>'
...
class LicenseTransaction(db.Model):
    ...
    __table_args__ = (db.CheckConstraint("license_type IN ('Fair','Good','Excellent')", name='check_transaction_license_type'), db.CheckConstraint("status IN ('pending','completed','failed')", name='check_transaction_status'), db.Index('idx_license_transactions_user', 'user_id', 'purchased_at'), db.Index('idx_license_transactions_expiry', 'license_end'))
    def __repr__(self): return f'<LicenseTransaction {self.license_type} for User {self.user_id}>'
...
class CommandUsage(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    __table_args__ = (db.UniqueConstraint('user_id', 'command_date', name='unique_user_command_date'), db.Index('idx_command_usage_user_date', 'user_id', 'command_date'), db.Index('idx_command_usage_date', 'command_date'), db.CheckConstraint('count >= 0', name='check_command_count_non_negative'), db.CheckConstraint('count <= 100', name='check_command_count_max'))
    def __repr__(self): return f'<CommandUsage User {self.user_id} Date {self.command_date}: {self.count}/100>'
...
class LoginAttempt(db.Model):
    ...
    user_agent = db.Column(db.String(500), nullable=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='SET NULL'), nullable=True)
    __table_args__ = (db.Index('idx_login_attempts_identifier_time', 'identifier', 'attempt_time'), db.Index('idx_login_attempts_ip_time', 'ip_address', 'attempt_time'), db.Index('idx_login_attempts_success_time', 'success', 'attempt_time'), db.CheckConstraint("attempt_type IN ('login','password_reset','api')", name='check_attempt_type'))
    def __repr__(self): return f'<LoginAttempt {self.identifier} Success={self.success} at {self.attempt_time}>'
...
class StoredOTP(db.Model):
    ...
    __table_args__ = (db.Index('idx_otp_type_used', 'otp_type', 'is_used'), db.Index('idx_otp_used_by', 'used_by'), db.Index('idx_otp_used_at', 'used_at'), db.CheckConstraint("credits_cost >= 0", name='check_otp_cost_non_negative'))

    def to_dict(self, admin_view=False):
        return {
            'id': self.id,
            'otp_code': self.otp_code if (admin_view or not self.is_used) else self.otp_code[:4] + '****',
            'otp_type': self.otp_type, 'otp_name': self.otp_name,
            'credits_cost': self.credits_cost, 'is_used': self.is_used,
            'used_by': self.used_by, 'used_by_username': self.user.username if self.user else None,
    ...
    def __repr__(self): return f'<StoredOTP {self.id}: {self.otp_name} ({self.otp_type}) Used={self.is_used}>'
...
def check_command_limit(user_id):
    ...
    if not usage: usage = CommandUsage(user_id=user_id, command_date=today, count=0); db.session.add(usage); db.session.commit()
...
def increment_command_count(user_id):
    today = datetime.utcnow().date()
    usage = CommandUsage.query.filter_by(user_id=user_id, command_date=today).first()
    if not usage: usage = CommandUsage(user_id=user_id, command_date=today, count=0); db.session.add(usage)
    usage.count += 1; db.session.commit()
    return usage.count
...
def check_login_limit(identifier, ip_address, max_attempts=10, window_hours=1):
    ...
    failed_attempts = LoginAttempt.query.filter(LoginAttempt.identifier == identifier, LoginAttempt.attempt_type == 'login', LoginAttempt.success == False, LoginAttempt.attempt_time >= cutoff_time).count()
    if failed_attempts >= max_attempts and user:
        suspended_until = datetime.utcnow() + timedelta(hours=window_hours)
        user.suspended_until = suspended_until; user.failed_login_count = failed_attempts; db.session.commit()
...
def log_login_attempt(identifier, success, ip_address, user_agent=None, user_id=None, attempt_type='login'):
    attempt = LoginAttempt(identifier=identifier[:255], attempt_type=attempt_type, success=success, ip_address=ip_address[:50], user_agent=user_agent[:500] if user_agent else None, user_id=user_id)
    db.session.add(attempt)
    if success and user_id:
        user = User.query.get(user_id)
        if user and user.suspended_until: user.suspended_until = None; user.failed_login_count = 0
    db.session.commit()
...
def get_user_command_stats(user_id, days=7):
    ...
    usage_records = CommandUsage.query.filter(CommandUsage.user_id == user_id, CommandUsage.command_date >= start_date).order_by(CommandUsage.command_date.desc()).all()
    ...
    return {'daily_usage': daily_usage, 'total_commands_last_7_days': total_commands, 'commands_today': today_usage, 'daily_limit': 100, 'remaining_today': max(0, 100 - today_usage)}
...
def run_migrations():
    try:
        ...
        for col_name, alter_statement in columns_to_add:
            try: db.session.execute(text(alter_statement)); db.session.commit(); print(f"✅ Added {col_name}")
            except Exception as e: print(f"⚠️ {col_name}: {e}"); db.session.rollback()
        try:
            db.session.execute(text("ALTER TABLE users DROP CONSTRAINT IF EXISTS check_license_type")); db.session.commit()
            db.session.execute(text("ALTER TABLE users ADD CONSTRAINT check_license_type CHECK (license_type IN ('None','Fair','Good','Excellent','Trial','Custom','12hr','24hr','2day','3day','7day'))")); db.session.commit()
            print("✅ Updated license_type constraint")
        except Exception as e: print(f"⚠️ license_type: {e}"); db.session.rollback()
        tables_to_create = [
            ('device_history', "CREATE TABLE IF NOT EXISTS device_history (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, device_id INTEGER REFERENCES devices(id) ON DELETE SET NULL, hardware_id VARCHAR(128), hwid_hash VARCHAR(256), device_name VARCHAR(255), action VARCHAR(50) NOT NULL, reason VARCHAR(255), ip_address VARCHAR(45), user_agent VARCHAR(500), extra_data JSONB, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP); CREATE INDEX IF NOT EXISTS idx_device_history_user_date ON device_history(user_id, created_at); CREATE INDEX IF NOT EXISTS idx_device_history_action ON device_history(action);"),
            ('user_sessions', "CREATE TABLE IF NOT EXISTS user_sessions (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, device_id INTEGER NOT NULL REFERENCES devices(id) ON DELETE CASCADE, session_token VARCHAR(256) UNIQUE NOT NULL, ip_address VARCHAR(45), user_agent VARCHAR(500), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, expires_at TIMESTAMP NOT NULL, last_activity TIMESTAMP DEFAULT CURRENT_TIMESTAMP, is_active BOOLEAN DEFAULT true); CREATE INDEX IF NOT EXISTS idx_sessions_token_expiry ON user_sessions(session_token, expires_at); CREATE INDEX IF NOT EXISTS idx_sessions_user_active ON user_sessions(user_id, is_active, expires_at);"),
            ('command_usage', "CREATE TABLE IF NOT EXISTS command_usage (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, command_date DATE NOT NULL, count INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, CONSTRAINT unique_user_command_date UNIQUE (user_id, command_date), CONSTRAINT check_command_count_non_negative CHECK (count >= 0), CONSTRAINT check_command_count_max CHECK (count <= 100)); CREATE INDEX IF NOT EXISTS idx_command_usage_user_date ON command_usage(user_id, command_date); CREATE INDEX IF NOT EXISTS idx_command_usage_date ON command_usage(command_date);"),
            ('login_attempts', "CREATE TABLE IF NOT EXISTS login_attempts (id SERIAL PRIMARY KEY, identifier VARCHAR(255) NOT NULL, attempt_type VARCHAR(50) DEFAULT 'login', attempt_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP, success BOOLEAN DEFAULT false, ip_address VARCHAR(50), user_agent VARCHAR(500), user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, CONSTRAINT check_attempt_type CHECK (attempt_type IN ('login','password_reset','api'))); CREATE INDEX IF NOT EXISTS idx_login_attempts_identifier_time ON login_attempts(identifier, attempt_time); CREATE INDEX IF NOT EXISTS idx_login_attempts_ip_time ON login_attempts(ip_address, attempt_time); CREATE INDEX IF NOT EXISTS idx_login_attempts_success_time ON login_attempts(success, attempt_time);"),
            ('stored_otps', "CREATE TABLE IF NOT EXISTS stored_otps (id SERIAL PRIMARY KEY, otp_code VARCHAR(128) UNIQUE NOT NULL, otp_type VARCHAR(50) NOT NULL, otp_name VARCHAR(100) NOT NULL, credits_cost INTEGER NOT NULL, is_used BOOLEAN DEFAULT false, used_by INTEGER REFERENCES users(id) ON DELETE SET NULL, used_at TIMESTAMP, notes TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, created_by INTEGER REFERENCES users(id) ON DELETE SET NULL, CONSTRAINT check_otp_cost_non_negative CHECK (credits_cost >= 0)); CREATE INDEX IF NOT EXISTS idx_otp_type_used ON stored_otps(otp_type, is_used); CREATE INDEX IF NOT EXISTS idx_otp_used_by ON stored_otps(used_by); CREATE INDEX IF NOT EXISTS idx_otp_used_at ON stored_otps(used_at);"),
        ]
        for table_name, create_statement in tables_to_create:
            try: db.session.execute(text(create_statement)); db.session.commit(); print(f"✅ Created {table_name}")
            except Exception as e: print(f"⚠️ {table_name}: {e}"); db.session.rollback()
        device_columns = [
        ...
        try:
            db.session.execute(text("UPDATE devices SET hwid_hash = encode(sha256(COALESCE(hardware_id,'')::bytea),'hex'), device_name = COALESCE(device_name,'Unknown Device'), is_active = COALESCE(is_active,true), is_bound = COALESCE(is_bound,true) WHERE hwid_hash IS NULL;")); db.session.commit()
            print("✅ Updated existing devices")
        except Exception as e: print(f"⚠️ devices update: {e}"); db.session.rollback()
        try:
            db.session.execute(text("DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname='check_transaction_type') THEN ALTER TABLE credit_transactions DROP CONSTRAINT check_transaction_type; END IF; IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname='credit_transactions_transaction_type_check') THEN ALTER TABLE credit_transactions DROP CONSTRAINT credit_transactions_transaction_type_check; END IF; END $$;")); db.session.commit()
            db.session.execute(text("ALTER TABLE credit_transactions ADD CONSTRAINT check_transaction_type CHECK (transaction_type IN ('admin_add','admin_deduct','purchase','usage','refund','commission','device_reset','pc_change','device_registration','credit_used','hwid_reset','otp_purchase'))")); db.session.commit()
            print("✅ Updated credit_transactions constraint")
        except Exception as e: print(f"⚠️ credit_transactions: {e}"); db.session.rollback()
